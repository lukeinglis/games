...
export function FeaturedGames() {
  ...
  const heroExternal = hero.external || hero.url.startsWith("http");
  ...
  const heroContent = (
    <>
      <div className="h-48 sm:h-56 relative overflow-hidden">
        ...
        {hero.hot && (
          <span className="absolute top-3 left-3 bg-action-red text-white text-[10px] font-bold px-2 py-0.5 rounded-sm uppercase tracking-wider z-10">
            HOT
          </span>
        )}
      </div>
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        ...
        <h2 className="font-heading text-xs text-gold/80 uppercase tracking-widest">
          Featured
        </h2>
            ...
            const cardContent = (
              <>
                <div className="h-28 sm:h-full min-h-[7rem] relative overflow-hidden">
                  ...
                  {game.hot && (
                    <span className="absolute top-2 right-2 bg-action-red text-white text-[10px] font-bold px-1.5 py-0.5 rounded-sm uppercase z-10">
                      HOT
                    </span>
                  )}
                </div>
                <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-navy via-navy/80 to-transparent">
                  <h3 className="font-heading text-[10px] sm:text-xs text-white group-hover:text-gold transition-colors leading-tight">
                    {game.title}
                  </h3>
                  <p className="text-[10px] text-gray-500 hidden sm:block">{game.tagline}</p>
                </div>
              </>
            );
